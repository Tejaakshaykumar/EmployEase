...
function FindEmployee() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/employeeDetails')
  ...
  const handleSearch = () => {
    const filtered = employeesData.filter((employee) => {
      ...
      return (
        ...
        employee.headquaters && employee.headquaters.toLowerCase().includes(location.toLowerCase()) &&
        matchesJobRole &&
        matchesCompany &&
        matchesCompanyType
      );
  ...
  return (
    <div className="find-job-container" style={{ minHeight: "80vh" }}>
      ...
      <div className="job-container">
          ...
          <ul className="job-list">
            {filteredEmployees.map((employee) => (
              <li key={employee._id} className="job-box">
                <div className='top'>
                  <p><img src={employee.logo ? `http://localhost:5000/${employee.logo.replace(/\\/g, '/')}`:(image)} alt="" />{employee.firstName} {employee.lastName}</p>
          ...
          <div className="job-details">
            {selectedEmployee ? (
              ...
              {/* <div className='banner' style={{
                backgroundImage: selectedEmployee.logo ? `url(http://localhost:5000/${selectedEmployee.banner.replace(/\\/g, '/')})` : (company),
                height: '40vh', 
                backgroundSize: 'cover',
                backgroundPosition: 'center',

              }}>
                <img src={selectedEmployee.logo ?`http://localhost:5000/${selectedEmployee.logo.replace(/\\/g, '/')}`:(image)} alt="Company Logo" 
                style={{
                  height:'20vh',
                  backgroundSize: 'cover',
                  borderRadius:'50%',
        ...
        >
          <img
            src={selectedEmployee.banner ? `http://localhost:5000/${selectedEmployee.banner.replace(/\\/g, '/')}` : company}
            alt="Company Banner"
            style={{
              width: '100%',
              height: '100%',
          ...
          />
          <img
            src={selectedEmployee.logo ? `http://localhost:5000/${selectedEmployee.logo.replace(/\\/g, '/')}` : image}
            alt="Company Logo"
            style={{
              position: 'absolute',
              bottom: '10px',
